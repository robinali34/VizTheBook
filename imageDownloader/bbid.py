#!/usr/bin/env python3
# from: https://github.com/ostrolucky/Bulk-Bing-Image-downloader
import os, urllib.request, re, threading, posixpath, urllib.parse, argparse, socket, time, hashlib, pickle, signal, imghdr, glob

#config
output_dir = '../assets/img/test' #default output dir
...
def download(pool_sema: threading.Semaphore, url: str, output_dir: str, keyword: str):
    ...
    try:
        ...
        if os.path.exists(os.path.join(output_dir, filename)) or len(glob.glob("../assets/img/test/{}.*".format(keyword))) > 0:
            return False
        # i = 0
        ...
        try:
            ...
            for index, link in enumerate(links):
                if limit is not None and current + index >= limit:
                    return
                result = download(pool_sema, link, output_dir, keyword)
                if result:
                    return
...
if __name__ == "__main__":
    ...
    if args.search_string:
        ...
        for keyword in inputFile.readlines():
            fetch_images_from_keyword(pool_sema, keyword,output_dir, args.filters, args.limit)
            backup_history()
            time.sleep(10)
        inputFile.close()